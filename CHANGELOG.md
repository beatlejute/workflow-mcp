...
- **workflow-ai — из реестра npm, `^1.12.0`.** `package-lock.json` держал его ссылкой на `../workflowAi`: сервер запускал раннер из рабочей копии соседнего репозитория, и любая правка там сразу попадала в работающие пайплайны. 1.8.0 — безынструментные агенты (`kind: http`) и обмен стадии с моделью (`model_io`). 1.8.1 — `PWD` агента равен его рабочему каталогу (Kilo-агенты тестов скилов работали в настоящем проекте), предупреждение о незацепившихся рельсах в тестах скилов, deep-research на рельсах. 1.9.0 — ответ агента без единого вызова инструмента на скиле с рельсами считается нарушением и повторяется; отказ рельс даёт готовую команду перехода; рельсы на Linux и macOS разбирают пути и программы PowerShell. 1.10.0 — судья тестов скилов Jev (модель решений через OpenRouter) с переоценкой неуверенных оценок CLI-судьёй, запись вызова судьи по каждой попытке, скрипт сравнения судей. 1.11.0 — судья тестов скилов — любой агент с командой; Jev подключён CLI-обёрткой с ключом в файле и стал судьёй всех скилов. 1.12.0 — ревью по собранным evidence: машиночитаемые проверки в DoD (`dod_format: 2`), исполнитель проверок, модель стадии ревью без инструментов через `model_io` для любого агента, тикет с зелёными проверками уходит в done без модели; таблице ревью в тексте тикета пайплайн больше не верит.
