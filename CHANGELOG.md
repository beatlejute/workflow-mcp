...
- **Каталог состояния зависел от регистра пути.** `path.resolve` регистр не меняет, и `d:\Dev` (так его передаёт клиент) и `D:\Dev` давали разные хеши: два каталога состояния и два разных `mcp_instance_id` на одну рабочую область. На Windows ключ рабочей области теперь считается по пути в нижнем регистре (`workspaceKey`). Прежняя проверка «case-insensitive on Windows» брала один путь и сверяла форму хеша — регистра не касалась вовсе.
  Смена ключа меняет и `mcp_instance_id`, поэтому маркер прогона, запущенного прежней версией, принимается на один переход по старому ключу (`legacyMcpInstanceId`). Без этого обновление посреди прогона делало его чужим: `foreign: true` в снимке и отказ `FOREIGN_PIPELINE` у `stop_pipeline` и `abort_pipeline` до вызова с `force`.
...
- `mcp_instance_id` считается по новому ключу (см. ниже). Маркер прежнего формата принимается на один переход, но сервер, который откатят на 1.x, свой же новый маркер уже не признает.
...
- Дополнены по итогам ревью: `state-dir` (+4, включая проверку самой изоляции), `list-running-pipelines` (+2), `git-client` (кеш `gh` проверяется через сам клиент, а не сравнением двух вызовов `machineStateDir`), новый `tests/watchers/relevant-file.test.mjs` (4).
- Полный прогон: 97 файлов, 1558 passed, 30 skipped, 4 todo, 0 failed.
...
| маркер прежнего формата больше не признаётся своим | 1 |
