...
- **Детекторы засоряли stderr каждый тик.** `[blocked_accumulation] directory does not exist` и `[retry_loop] counter limit not defined` печатались по разу на тик на проект, хотя описывают обычное состояние проекта, а не ошибку. Пока цикл не запускался, этого никто не видел.
...
- `tests/health/detectors/crashed.test.mjs` переписан под lock раннера: 19 проверок вместо 22, включая четыре варианта испорченного lock'а (`empty`, `garbage`, `no-pid`, `bad-pid`) и приоритет `run_id` из lock'а над именем лога.
