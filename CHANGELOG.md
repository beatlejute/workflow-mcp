...
Правки по ревью 3.2.5. Два пункта по существу, один — ложное утверждение в самом CHANGELOG.
...
Саботаж на наборе из 115 проверок: сбой запуска утилиты снова разбирается — 1 падение, текст снова важнее живости — 3, код возврата читается после живости — 1, принудительный путь `abort` не разбирает отказ — 1, `checkProcess` дословно старый — 1, `checkProcess` отвечает из памяти — 1, lock без времени снова не проверяется — 2.
...
Саботаж на наборе из 123 проверок: lock без времени снова не проверяется — 2 падения, `checkProcess` отвечает из памяти — 1, код возврата `taskkill` игнорируется — 1, живость на принудительном пути не спрашивают — 2, мягкая попытка решает по живости — 4.
