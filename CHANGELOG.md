...
Саботаж на наборе из 82 проверок: `force` снова смотрит на причину отказа — 5 падений, `foreign` снова берётся из причины — 1, префикс логгера снова любой длины — 2, срок жизни записи игнорируется — 1, эскалация решается по памяти — 2, `resume` решает по памяти — 4.
...
- **`force: true` у `stop_pipeline` не отменяет отказ `PID_REUSED`.** `force` снимает вопрос о том, чей это пайплайн, но не вопрос о том, есть ли он вообще. Подсказка отказа прямо просит не повторять с `force`; обходить её же было странно. Аварийный ключ `WORKFLOW_MCP_FORCE_FOREIGN=1` по-прежнему снимает проверку владения целиком — это ключ на крайний случай, и в README так и написано.
...
- Запуск без `started_by: 'mcp'` в lock'е не считается своим — ни по какой причине, кроме аварийного ключа `WORKFLOW_MCP_FORCE_FOREIGN=1`, который снимает проверку владения целиком. Прежде лежащий рядом маркер «доказывал» владение пайплайном, у которого источник в lock'е не проставлен вовсе.
