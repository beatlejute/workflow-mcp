...
- Дополнены по итогам ревью: `state-dir` (+4, включая проверку самой изоляции), `list-running-pipelines` (+1 тест и проверка `foreign` у живого прогона), `marker` (+3 на переходную сверку), `git-client` (кеш `gh` проверяется через сам клиент, а не сравнением двух вызовов `machineStateDir`), новый `tests/watchers/relevant-file.test.mjs` (4).
...
- `marker` больше не зависит от окружения запускающего: `MCP_CWD` фиксируется на каталог теста. Прежде набор молча опирался на то, что в пути временного каталога есть заглавные буквы.
- Полный прогон: 97 файлов, 1561 passed, 30 skipped, 4 todo, 0 failed.
...
(66 проверок): отказ принимать маркер прежнего формата валит 3, возврат
сверки мимо переданного ожидания — 2.

### Ops
- Разово убраны накопленные каталоги состояния: 15 789 пустых и 465 с одним лишь кешем `gh` в первый заход, ещё 19 пустых — после того, как набор тестов перестал их плодить. Остались два каталога с историей алертов: по старому ключу рабочей области и по новому. Смена ключа (регистр) один раз обнуляет дедуп: повтор каждого живого алерта уйдёт клиенту заново.
