...
- **Состояния `aborting` и `killed` не возникали никогда.** `list_running_pipelines` и ресурс `workflow://pipeline-state` искали файлы `.workflow/logs/.aborting` и `.killed`, которых не пишет ни сервер, ни раннер workflow-ai, ни расширение VS Code. README перечислял оба состояния среди возможных. Тот же класс фантома, что убранный `.runner-pids`.
  ...
  Состояние `killed` маркера не требовало вовсе: оно выводится из ненулевого кода выхода в логе. Чтение `.killed` убрано, само состояние осталось.
- **`stale` затирало разбор лога.** Мёртвый pid при живом lock'е давал `stale` раньше, чем кто-либо смотрел на код выхода, поэтому `killed` было недостижимо и после исправления чтения. Теперь конец прогона, записанный в лог, важнее: `stale` остаётся для случая «lock пережил процесс, а чем кончилось — неизвестно». Признак `stale_lock` не изменился.
...
- `tests/tools/list-running-pipelines.test.mjs`: два теста фантомных маркеров заменены шестью настоящими — флаг даёт `aborting`, флаг от другого pid и от другого `run_id` игнорируется, протухший игнорируется, abort перекрывает паузу, ненулевой код выхода даёт `killed`.
