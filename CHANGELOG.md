...
- **workflow-ai — из реестра npm, `^1.9.0`.** `package-lock.json` держал его ссылкой на `../workflowAi`: сервер запускал раннер из рабочей копии соседнего репозитория, и любая правка там сразу попадала в работающие пайплайны. 1.8.0 — безынструментные агенты (`kind: http`) и обмен стадии с моделью (`model_io`). 1.8.1 — `PWD` агента равен его рабочему каталогу (Kilo-агенты тестов скилов работали в настоящем проекте), предупреждение о незацепившихся рельсах в тестах скилов, deep-research на рельсах. 1.9.0 — ответ агента без единого вызова инструмента на скиле с рельсами считается нарушением и повторяется; отказ рельс даёт готовую команду перехода; рельсы на Linux и macOS разбирают пути и программы PowerShell.
