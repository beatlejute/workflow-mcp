...
Саботаж измерен на названном наборе — `tests/process/{taskkill-classify,abort-escalation-verdict,control,control-windows-mock,run-lock}` и `tests/health/pid-probe`, 104 проверки: сбой запуска утилиты снова разбирается — 1 падение, текст возвращён перед живостью — 2, живость не спрашивается вовсе — 5, код возврата читается после живости — 1, силовой путь `abort` отдаёт сырой отказ — 1, `checkProcess` дословно из `549a310` — 1, `checkProcess` отвечает из памяти — 1, lock без времени снова не проверяется — 2, сбой `tasklist` снова значит «мёртв» — 7.

Прежняя редакция этой записи называла 3 падения для «текст важнее живости». Число не воспроизводится: буквально этот саботаж даёт 2 (после добавления второй проверки порядка), а 5 получаются при удалении живости целиком — это другой случай, и он теперь в списке отдельно.
...
Саботаж на наборе `tests/process/*` плюс `tests/health/pid-probe` (123 проверки): lock без времени снова не проверяется — 2 падения, `checkProcess` отвечает из памяти — 1, код возврата `taskkill` игнорируется — 1, живость на принудительном пути не спрашивают — 2, мягкая попытка решает по живости — 4.
