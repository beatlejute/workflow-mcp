...
- **workflow-ai — из реестра npm, `^1.8.0`.** `package-lock.json` держал его ссылкой на `../workflowAi`: сервер запускал раннер из рабочей копии соседнего репозитория, и любая правка там сразу попадала в работающие пайплайны. 1.8.0 — безынструментные агенты (`kind: http`) и обмен стадии с моделью (`model_io`).
