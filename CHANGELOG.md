...
- **`checkProcess` был четвёртой реализацией живости** — со своим прочтением `EPERM` (как `UNKNOWN_ERROR`, то есть «процесса нет») и без Windows-ветки. Свёрнут в обёртку над общей проверкой; ответ берётся свежим, потому что зовут его сразу после сигнала.
...
Саботаж на наборе из 123 проверок: lock без времени снова не проверяется — 2 падения, `checkProcess` со своей логикой — 1, `checkProcess` отвечает из памяти — 1, код возврата `taskkill` игнорируется — 1, живость на принудительном пути не спрашивают — 2, мягкая попытка решает по живости — 4.

Полный прогон: 106 файлов, 1637 passed, 0 failed.
