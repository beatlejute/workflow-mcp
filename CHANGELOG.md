...
Саботаж на наборе из 78 проверок: помощник никогда не отказывает — 17 падений, `pause` без проверки — 4, `resume` — 2, `abort` — 3, `stop` — 8, эскалация — 1, `EPERM` снова как «процесса нет» — 1.
...
- **`force: true` у `stop_pipeline` не отменяет отказ `PID_REUSED`.** `force` снимает вопрос о том, чей это пайплайн, но не вопрос о том, есть ли он вообще. Подсказка отказа прямо просит не повторять с `force`; обходить её же было странно. (В 3.2.1 то же сделано для аварийного ключа и для остальных трёх операций.)
...
- Запуск без `started_by: 'mcp'` в lock'е не считается своим — ни по какой причине, кроме аварийного ключа `WORKFLOW_MCP_FORCE_FOREIGN=1`, который снимает проверку владения (но с 3.2.1 не проверку переиспользованного номера). Прежде лежащий рядом маркер «доказывал» владение пайплайном, у которого источник в lock'е не проставлен вовсе.
