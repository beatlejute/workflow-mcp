...
- Проверки новых защит: `tests/e2e/health-alerts.test.mjs` поднимает сервер с непригодным `state.dir` и требует работающий `tools/list`; `tests/health/publisher.test.mjs` — отказ `appendFileSync` не отменяет уведомление; `tests/health/service.test.mjs` — исключение внутри тика не убивает службу и следующий тик проходит.
