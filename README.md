...
> его не снимает — `stop_pipeline` откажет и с ним. Такой lock нужно удалить руками:
> сервер его не сносит ни в `stop`, ни в `start_pipeline` — ошибка проверки стоила бы
> второго пайплайна поверх живого. Переменная `WORKFLOW_MCP_FORCE_FOREIGN=1` снимает
> проверку владения целиком, включая и этот случай: это аварийный ключ, а не рабочий
> режим.
>
