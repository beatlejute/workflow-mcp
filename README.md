...
- Обход проектов синхронный: на каждый тик приходится до одного вызова `tasklist` на проект с живым lock'ом и до одного `git status` на git-проект, каждый с таймаутом в 5 секунд. На медленной файловой системе это задерживает ответы сервера — при большом числе проектов увеличивайте `tick_interval_sec`.
...
- `list_running_pipelines()` — все идущие пайплайны по обнаруженным проектам: `state` (`running|paused|aborting|killed|completed|stale`), текущая стадия, номер шага, `awaiting_approval`, `marker_valid`, а также `foreign`, `stale_lock` и `marker_reason`, когда они применимы. Параметров нет.
  (Оговорка: состояния `aborting` и `killed` в текущей версии не возникают — файлы-маркеры `.workflow/logs/.aborting` и `.killed` читаются, но не пишутся ни сервером, ни раннером, ни расширением; решение зафиксировано в PLAN-001.)
