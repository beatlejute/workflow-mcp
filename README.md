...
- `list_running_pipelines()` — все идущие пайплайны по обнаруженным проектам: `state` (`running|paused|aborting|killed|completed|stale`), текущая стадия, номер шага, `awaiting_approval`, `marker_valid`, а также `foreign`, `stale_lock` и `marker_reason`, когда они применимы. Параметров нет.
  Откуда берутся состояния: `aborting` — пока идёт `abort_pipeline` (флаг `.workflow/state/abort-state.json` живёт всё grace-окно и сверяется с pid и `run_id` текущего прогона); `paused` — файл паузы с тем же pid или ожидающее одобрение; `running` — живой pid раннера; `completed` и `killed` — по коду выхода в логе (`0` и не `0`); `stale` — lock пережил процесс, а конца прогона в логе нет.
