...
Подписчик узнаёт об обеих сторонах: появление условия шлёт `resources/updated` через дедуп публикации, исчезновение — через сравнение отпечатков с прошлым тиком. То есть подписка отстаёт от чтения не больше, чем на `tick_interval_sec`.
