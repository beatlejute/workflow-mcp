...
> **Оговорка.** Состояния `aborting` и `killed` в текущей версии не возникают: файлы-маркеры `.workflow/logs/.aborting` и `.killed` читаются, но не пишутся ни сервером, ни раннером, ни расширением. Либо их начнут писать `abort_pipeline` и `stop_pipeline`, либо состояния уберут — решение зафиксировано в PLAN-001.
