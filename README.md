...
`workflow://alerts` — не проверка состояния на момент запроса, а выжимка из истории: последняя запись по каждому отпечатку за сутки. Условие, которое уже разрешилось, остаётся в списке, пока запись не состарится. Понять, идёт ли что-то прямо сейчас, можно по `workflow://pipeline-state` или `list_running_pipelines` — они строятся заново на каждое чтение.
