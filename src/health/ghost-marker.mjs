...
export function buildGhostMarkerMatcher(marker) {
  ...
  //
  // Префикс описан точно, а не «сколько угодно скобочных групп»: у раннера он
  // ровно один и жёсткий — `[время] [УРОВЕНЬ] [стадия]`. Свободное количество
  // групп пропускало дословную цитату лог-строки внутри вывода агента
  // (`[…] [INFO] [claude]   [время] [INFO] [verify-artifacts]   [GHOST-…]`),
  // а агенты цитируют `log_excerpt` из `list_ghost_executions` именно так.
  const logPrefix = '(?:\\[[^\\]]*\\]\\s+\\[[A-Z]+\\]\\s+\\[[^\\]]*\\]\\s+)?';
