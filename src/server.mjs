...
async function main() {
  ...
  // Каталог состояния: там лежит история алертов — ресурс
  // `workflow://alerts/history`. Сам `workflow://alerts` от каталога не
  // зависит: он отвечает обходом детекторов. На read-only дереве служба
  // здоровья работает без истории, алерты уходят уведомлениями.
  //
