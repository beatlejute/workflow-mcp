 ...
 * закреплены три довода по порядку: код возврата, живость, текст. Живость
 * стоит раньше текста намеренно: текст локализован, факт — нет.
 *
...
describe('текст — последний довод, когда факта нет', () => {
  it('английские сообщения распознаются, когда живость не спрашивают', () => {
...
describe('живость важнее текста и спрашивается только на принудительном пути', () => {
  ...
  it('английское «процесс не найден» на живом процессе тоже не перебивает факт', async () => {
    // Обратная сторона: текст говорит «нет процесса», а процесс есть.
    victim = await spawnVictim();

    expect(classifyTaskkillFailure(
      { exitCode: 1, stderr: 'ERROR: The process "node.exe" not found.' },
      victim.pid,
      { probeLiveness: true }
    )).toBe('PERMISSION_DENIED');
  });
